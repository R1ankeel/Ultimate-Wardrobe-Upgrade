...
- Phase 6 (cont.) Sprint 6.3 done - donor library screen (`DonorLibraryViewModel` + `DonorLibraryView`): donor table (file name, `Kind`, ProvidedSets count, BodySlide/physics, date, Remove/Reclassify/Set Kind) + drop-zone import (`.7z/.zip/.rar`) through the Phase 2 pipeline on `IBackgroundTaskService` via the `IDonorImportRunner` seam, failure -> typed alert with the library snapshot unchanged and every mutation autosaved through the shared `IProjectSession.Store`. Sprint 6.4 done - the mapping matrix grid (`OverhaulViewModel`/`OverhaulMatrix` on `OverhaulView`): FEMALE ARMOR then MALE ARMOR sections of the catalog sets (a set with both genders appears in both via its variants), one column per distinct weight class present (Heavy/Light/Clothing/n/a), and one cell per (set, gender, weight). A mapped cell raises a card (set name + one line per distinct base donor + one per `BodyConversionPatch`/`PhysicsPatch`); a missing variant or an unmapped variant renders blank/empty. Set status via `MappingService.GetArmorSetStatus`, header label via `GetOverhaulProgress`; search filters the row band and a status filter highlights matching rows. Rows are `IReadOnlyList` projections (never an `ObservableCollection` matrix rebuild) for 2-D virtualization, cell cards virtualized in the view. The selected Overhaul is pinned cross-navigation by the new App-layer singleton `IOverhaulSelection`/`OverhaulSelection` (`OverhaulId`), since the page is recreated from DI per navigation. 11 new headless `OverhaulViewModelTests`; full suite 654 tests green, Release 0 warnings/0 errors. `Done` remains a caller-side overlay (Sprint 3.3) and is not toggled by the edit flow. Sprint 6.5 done - the anchored-popover single-cell editor (`ArmorSetDetailViewModel` rescoped, hosted by a WPF `Popup` in `OverhaulView`): one editor bound per activated (set, gender, weight) `Variant` + its `PieceMapping`s, opened by `OverhaulViewModel.Activate` (which resolves the variant directly from `cell.Set` because an unmapped cell is blank, `Variant` null) and cleared on close; `PieceEditRowViewModel` rows filter the "load armor" donors by the pure `DonorCompatibility` predicate (same gender/weight or Unisex/Any) and offer BodyConversion/Physics patch dropdowns (by `DonorAssetKind`) only when a row reads `NeedsPatch`; the Phase 3 command set (`AssignDonor`/`AttachBodyPatch`/`AttachPhysicsPatch`/`Unassign`/`DetachBodyPatch`/`DetachPhysicsPatch`/`SetNotes`) runs verbatim over `MappingService` with the derived status (`GetStatus` under `Overhaul.Policy`) driving the highlight; every edit triggers `RecomputeMatrix` in the host + an `IProjectSession.Store.SaveAsync` flush (amendment 3 autosave) with a guaranteed flush on close. 9 new headless `ArmorSetDetailViewModelTests`; full suite 663 tests green, Release 0 warnings/0 errors. Sprint 6.6 done - export + polish: `ExportViewModel`/`ExportView` run the EXISTING Phase 5 `IPatcher` unchanged (`WardrobePatcher.BuildAsync` on `IBackgroundTaskService` with `IProgress<PatchProgress>` + `CancellationToken` cancellation; the `PatchResult` is captured via a closure because `RunAsync` returns `Task`) with a pre-export checklist (`GetOverhaulProgress` status chip counts + an allow-partial switch), default output folder `<Project.Root>/Export`, progress bar + cancel, a result card from `PatchResult`/`PatchReport` (mod folder, plugin path, overridden records, copied files/bytes, warnings list), open-in-Explorer and re-export (Phase 5 `OutputFolder.ClearModDir` delete-then-rebuild makes re-export clean); failures surface `IAppDialogService.AlertAsync`, `OperationCanceledException` -> canceled snackbar. Polish (roadmap 8.5): `IThemeService` (UI `WpfUiThemeService` -> `ApplicationThemeManager.Apply` + persisted via `RecentProjectsStore.GetThemeMode`/`SetThemeMode` in the same `%LocalAppData%` settings.json, headless `NullThemeService`) exposed on `MainViewModel` as `IsDarkTheme`/`ThemeLabel`/`ToggleThemeCommand`; a status-glyph legend (`StatusLegend` + `StringToSymbolConverter`) in `OverhaulView`; an app icon + version title; monospace paths + empty states + keyboard nav. 13 new headless tests (9 `ExportViewModelTests` incl. re-export/cancel/failure, +4 theme `RecentProjectsStoreTests`); full suite 676 tests green, Release 0 warnings/0 errors. Manual E2E (roadmap 6.8/10.4) recorded in the Done note: run headless over real data - real `D:\Skymod\Stock Game` scan + six real `ModsForTests\Armor` donors (incl. "Red Hood - HIMBO" `BodyConversionPatch` with 2 BodySlide + 10 physics) + six real vanilla set mappings (real HIMBO body-conversion + physics patches attached) + the real `WardrobePatcher` -> esp reopened under Mutagen (ESL on, masters = Dawnguard/Dragonborn/Skyrim.esm), `meta.ini` present with "6 sets mapped.", mod folder == exactly the 82 copied files + esp + meta.ini; all temp folders cleaned. Post-6.6 crash hotfixes - (1) the shell crashed after the picker created/opened a project (`XamlParseException: AppIcon` not found): the root-element icon StaticResource could not see the window's own resources, fix = app-scope icon applied from code (`MainWindow.ApplyAppIcon`), regression `MainWindowBootTests`; (2) the Overhaul screen crashed on open (`XamlParseException: "Auto,0,0,0" is not a valid value for "Margin"`) - the popover "Import patch" button used a Grid length where a `Thickness` is required, fix = `Margin="12,0,0,0"`, regression `OverhaulViewBootTests`; full suite 678 tests green, Release 0 warnings/0 errors. Sprint 6.6 done - the export screen (`ExportViewModel`/`ExportView` running the EXISTING Phase 5 `IPatcher` unchanged over `IBackgroundTaskService` with progress/cancel/result card/re-export) and polish (dark/light theme toggle persisted via `IThemeService`, status-glyph legend + `StringToSymbolConverter`, app icon + version title); full suite 676 tests green, Release 0 warnings/0 errors. Post-6.6 shell-crash hotfix (v0.6.6.1) - the shell crashed with `XamlParseException: Cannot find resource named "AppIcon"` the moment a project was created/opened because `MainWindow.xaml` referenced `{StaticResource AppIcon}` on its root while the resource lived in the window's own later-declared resources; the icon now lives at app scope (`App.xaml`) and is applied from code (`MainWindow.ApplyAppIcon`, null-safe `TryFindResource`) - regression covered by `MainWindowBootTests`; full suite 677 tests green, Release 0 warnings/0 errors.
