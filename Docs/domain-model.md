...
- Phase 6 (cont.) Sprint 6.3 done - donor library screen (`DonorLibraryViewModel` + `DonorLibraryView`): donor table (file name, `Kind`, ProvidedSets count, BodySlide/physics, date, Remove/Reclassify/Set Kind) + drop-zone import (`.7z/.zip/.rar`) through the Phase 2 pipeline on `IBackgroundTaskService` via the `IDonorImportRunner` seam, failure -> typed alert with the library snapshot unchanged and every mutation autosaved through the shared `IProjectSession.Store`. Sprint 6.4 done - the mapping matrix grid (`OverhaulViewModel`/`OverhaulMatrix` on `OverhaulView`): FEMALE ARMOR then MALE ARMOR sections of the catalog sets (a set with both genders appears in both via its variants), one column per distinct weight class present (Heavy/Light/Clothing/n/a), and one cell per (set, gender, weight). A mapped cell raises a card (set name + one line per distinct base donor + one per `BodyConversionPatch`/`PhysicsPatch`); a missing variant or an unmapped variant renders blank/empty. Set status via `MappingService.GetArmorSetStatus`, header label via `GetOverhaulProgress`; search filters the row band and a status filter highlights matching rows. Rows are `IReadOnlyList` projections (never an `ObservableCollection` matrix rebuild) for 2-D virtualization, cell cards virtualized in the view. The selected Overhaul is pinned cross-navigation by the new App-layer singleton `IOverhaulSelection`/`OverhaulSelection` (`OverhaulId`), since the page is recreated from DI per navigation. 11 new headless `OverhaulViewModelTests`; full suite 654 tests green, Release 0 warnings/0 errors. `Done` remains a caller-side overlay (Sprint 3.3) that the 6.5 popover will toggle via `SetDone`.
