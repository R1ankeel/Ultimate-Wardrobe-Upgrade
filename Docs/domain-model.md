...
- Phase 3 - `UltimateWardrobe.Mapping` implements the manual mapping `MappingService` over the existing `PieceMapping`/`ArmorSetStatus` domain (Sprints 3.0-3.3 done): it assigns a donor per target piece/gender, attaches body/physics patch layers, derives `NeedsPatch` from the combined donor-or-patch `Detected*` flags plus `Overhaul.Policy` (via `NeedFor`/`BodyMarkerFromPath`), recommends patches, computes per-set `ArmorSetStatus` and Overhaul progress, and toggles the caller-side `Done` overlay via `SetDone` (`Done` is an overlay, not a derived status). Core gained the additive `PatchPolicy` enum + `Overhaul.Policy`. Workspace pending: real-donor spot-check + docs (3.4). See `Docs/mapping.md`.
