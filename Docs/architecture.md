...
├── src/UltimateWardrobe.Patcher     # Phase 5 ESP patcher + file slicer (Sprints 5.0-5.2 done - Core IPatcher amendment + TargetResolver + PatchException + PluginBuilder override records/ESL gate/loose-path skip + DonorFileLocator + FileSlicer whitelist slice/overlay/dedup; docs in Docs/patcher.md)
...
- `Docs/patcher.md` - ESP patcher + target resolution + plugin builder + file slicer (Phase 5; Sprints 5.0-5.2 done - `TargetResolver` + Core `IPatcher`/`PatchReport` amendment + `PluginBuilder` override records/ESL gate/amendment #6 loose-path skip + `DonorFileLocator` + `FileSlicer` whitelist slice/patch overlay/whole-export dedup)
- `Plans/phase5.md` - Phase 5 plan, Sprints 5.0-5.2 done (Scaffolding + Core `IPatcher` amendment + `TargetResolver` + `PluginBuilder` + `FileSlicer`/`DonorFileLocator`)
