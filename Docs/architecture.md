...
├── src/UltimateWardrobe.Persistence # Phase 4 SQLite (Sprints 4.0-4.3 done - project + Microsoft.Data.Sqlite + ProjectDatabase WAL/foreign_keys bootstrap + Migrator/M001_Initial schema + .bak backup + UnitOfWork re-issuing PRAGMA defer_foreign_keys=ON per transaction + RowCodecs + 5 repositories + PersistenceJson row codecs + IProjectStore.SaveAsync/LoadAsync whole-graph round-trip; Core 4.0.2 amendment: Overhaul.CreatedAt/ModifiedAt; Core 4.3 amendment: Overhaul.Catalog)
