...
├── src/UltimateWardrobe.App         # (Phase 6 - WPF shell) generic-host composition root + ProjectPickerWindow startup gate (amendment 7) + FluentWindow/NavigationView shell + StatusBar + Project/Overhaul screens (Sprints 6.1-6.2 done: DI smoke green + picker/create/open + overhaul cards, WPF-UI 4.3.0 wiring + spike in Docs/app.md; donor import + matrix grid + export land in Sprints 6.3-6.6)
...
- `Docs/app.md` - WPF shell: composition root + host lifecycle + startup `ProjectPickerWindow` gate (single project per process, amendment 7) + project/overhaul management (picker create/open/recent, overhaul cards, shared-store autosave) + WPF-UI 4.3.0 wiring + spike conclusions (Phase 6, Sprints 6.1-6.2 done; donor import + matrix grid + export land in Sprints 6.3-6.6)
