...
- `Docs/patcher.md` - ESP patcher + target resolution + plugin builder + file slicer + orchestrator (Phase 5; Sprints 5.0-5.3 done - `TargetResolver` + Core `IPatcher`/`PatchReport` amendment + `PluginBuilder` override records/ESL gate/amendment #6 loose-path skip + `DonorFileLocator` + `FileSlicer` whitelist slice/patch overlay/whole-export dedup + `OutputFolder` mod folder/meta.ini + `WardrobePatcher` orchestrator)
- `Plans/phase5.md` - Phase 5 plan, Sprints 5.0-5.3 done (Scaffolding + Core `IPatcher` amendment + `TargetResolver` + `PluginBuilder` + `FileSlicer`/`DonorFileLocator` + `OutputFolder`/`WardrobePatcher`)
