...
- F4 clothing grouping suite (`SyntheticClothingUniverse.cs` + `ArmorSetGrouperClothingTests`) - a 98-item synthetic plugin (34 Clothes + 14 Robes + 15 Hoods + 33 Shoes + 2 Gloves from the spec) with distinct BOD2 slots and distinct mesh subfolders, proving that Belted Tunic etc. do not collapse via the bare `clothes` KeyNormalizer key into a megaset, that robes/hoods/shoes/gloves remain separate from body clothes, and that no set exceeds 5 members; plus an Integration-gated real-game guard that max set size stays <= 150 and GroupedSets stays > 50, locking the F4 megaset invariant.
