...
- Sprint 6.8 filter suite (`SyntheticFilteringUniverse.cs` + `ArmorSetGrouperTests`) - a cheap case-insensitive plugin (ring, amulet, circlet, enchanted-name records incl. `&`-combined phrases and lowercase suffixes, plain cuirass) proves rings/necklaces skip as `Jewelry`, a circlet stays in the catalog, vanilla-enchanted names skip as `Enchanted`, plain armor survives, and per-reason skip counts track exactly.
