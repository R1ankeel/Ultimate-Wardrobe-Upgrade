...
- **Layout.** Two-column `Grid`: LEFT column "ARMOR 1" is a read-only piece inventory - one `PieceInventoryRowViewModel` per variant piece (EditorId, slot, target mesh, per-piece donor badges) - and RIGHT column "ARMOR 2" edits the single replacement. `AvailableDonors` (`FullReplacer` + gender-compatible only, weight ignored since F1, current donor excluded) drives a ComboBox; `LoadDonorLabel` reads "Load Armor" (nothing loaded) or "Change" (`HasCurrentDonor`), `CurrentDonorText` reads "Armor: <mod name>" / "Nothing loaded yet". "Load Armor" now also works WITHOUT a ComboBox selection (Sprint 6.10): `LoadDonorCommand` is an `IAsyncRelayCommand` that assigns the selected donor when one is active, otherwise opens a mod-archive picker (`IAppDialogService.PickModArchiveAsync`, `.7z/.zip/.rar`), imports the archive through `IDonorImportRunner`, and assigns its `FullReplacer` donor in ONE step; an imported kind that is not a compatible `FullReplacer` alerts but keeps the asset in the donor library. The donor piece for each target piece is resolved via `DonorCompatibility.FindDonorPiece` (gender-only via `SlotNormalizer`, weight ignored since F1, strict slot - any donor piece whose canonical slot matches the target slot may back it: `32 Body` = `Cuirass`/`Armor`/`Robe`/`Dress`, `33 Hands`/`34 Forearms` = `Gauntlets`/`Bracers` family, `37 Feet` = `Boots`/`Shoes`/`Sandals`, `30 Head`/`31 Hair`/`42 Circlet` etc. Head family - any Heavy donor may back a Light/Clothing variant when the biped slot matches; a missing slot returns null so the editor warns per-piece and allows partial assignment for the remaining slots) and assigned through the Phase 3 `MappingService.AssignDonor` verbatim (a new donor REPLACES the variant's `PieceMapping`s, which clears stale patch layers automatically). `SlotNormalizer` (`src/UltimateWardrobe.App/ViewModels/SlotNormalizer.cs:1`) bridges Branch-1 frozen slots and Branch-2 word slots.
- **Body / physics checks (Sprint 6.9 T2, weight-agnostic since F1, tightened F3).** Required body type is gender-driven - `DonorCompatibility.RequiredBodyTypeFor`: female -> 3BA, male -> HIMBO. `DonorContainsBody(donor, type)` (F3) is true only when a provided mesh path - or a detected BodySlide file path - carries the required body marker via `MappingService.BodyMarkerFromPath` (`3ba`/`himbo` tokens); a generic `DetectedBodySlideFiles.Count > 0` alone no longer satisfies a female 3BA check when the donor's meshes are `himbo` and vice versa, enforcing per-gender patch demand. `DonorHasPhysics(donor)` reads `DetectedPhysicsFiles`. Weight is ignored - the same check applies whether the target is Heavy, Light or Clothing. Check lines render text status ("3BA: OK" / "HIMBO: OK" / "HDT-SMP: OK" - plain text, no glyphs, per docs convention) when the donor already satisfies a check; otherwise a conditional dropdown row is offered ("Load 3BA patch"/"Load HIMBO patch"/"Load HDT-SMP patch") that picks ONE `BodyConversionPatch`/`PhysicsPatch` asset (`BodyPatches`/`PhysicsPatches`, seeded by `SelectPatchesForCurrentState`) attached set-level via `MappingService.AttachPatch`; an attached patch gets a Clear button (`ClearBodyPatchCommand`/`ClearPhysicsPatchCommand` -> `DetachPatch`). F1 guarantees any donor of the correct gender can be evaluated, regardless of its `WeightClass`; F3 guarantees a male HIMBO mesh does not satisfy a female 3BA requirement.
