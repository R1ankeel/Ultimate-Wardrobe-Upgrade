...
## F2 slot normalization (App layer)

`SlotNormalizer` (`src/UltimateWardrobe.App/ViewModels/SlotNormalizer.cs:1`) is the F2 bridge that makes mesh-only donors interchangeable with plugin donors. It strips the frozen numeric prefix (`"32 Body"` -> `"Body"`), maps word synonyms from `PieceTypeDetector.EquipmentWords` to a single canonical id (`Cuirass`/`Armor`/`Clothes`/`Robe`/`Dress` -> `Body`, `Gauntlets`/`Gloves` -> `Hands`, `Bracers` -> `Forearms`, `Boots`/`Shoes`/`Sandals` -> `Feet`, `Helmet`/`Hood`/`Crown` -> `Head`/`Hair`/`Circlet` family), and allows family fallback for `Hands`/`Forearms`, `Feet`/`Calves` and the `Head` family (`Head`/`Hair`/`LongHair`/`Circlet`/`Ears`). `DonorCompatibility.FindDonorPiece` (`src/UltimateWardrobe.App/ViewModels/DonorCompatibility.cs:53`) now uses `SlotNormalizer.AreCompatible` and returns `null` on a missing slot - the editor (`src/UltimateWardrobe.App/ViewModels/ArmorSetDetailViewModel.cs:339`) warns per-piece and allows partial assignment for the remaining slots instead of falling back to the first piece. Weight remains ignored (F1) - any `Heavy` donor may back a `Light`/`Clothing` target when the canonical slot matches, and a 1-piece `Clothes` variant maps with a single `AssignDonor`.

## F3 patch row polish (App layer)

`DonorCompatibility.DonorContainsBody` (`src/UltimateWardrobe.App/ViewModels/DonorCompatibility.cs:97`) is now strict (F3): it returns true only when a provided mesh path - or a detected BodySlide file path - carries the required body marker via `MappingService.BodyMarkerFromPath` (`3ba` -> ThreeBA, `himbo` -> HIMBO). A generic `DetectedBodySlideFiles.Count > 0` no longer satisfies a female `3BA` check when the donor's meshes are `himbo` and vice versa. `DonorHasPhysics` stays `DetectedPhysicsFiles.Count > 0`. `ArmorSetDetailViewModel` keeps `RequiredBodyName` (`3BA` vs `HIMBO` at `src/UltimateWardrobe.App/ViewModels/ArmorSetDetailViewModel.cs:125`), `BodyCheckText` (`"3BA: OK"` / `"HIMBO: OK"` / `"HDT-SMP: OK"` at `src/UltimateWardrobe.App/ViewModels/ArmorSetDetailViewModel.cs:134` with short dash "-"), and `LoadBodyPatchLabel` (`"Load 3BA patch"` / `"Load HIMBO patch"` at `src/UltimateWardrobe.App/ViewModels/ArmorSetDetailViewModel.cs:157`) with `ShowBodyPatchRow` / `ShowClearBodyPatch` driving the anchored popover (`src/UltimateWardrobe.App/Views/OverhaulView.xaml:307`). A Female Iron editor with a Gryphon donor (no `3BA` token, no BodySlide marker) correctly shows `3BA: patch required` with `[Load 3BA patch]` and `HDT-SMP: patch required` with `[Load HDT-SMP patch]`; a Female Ciri donor with `3ba` mesh and hdt shows both check - male shows `HIMBO` not `3BA`.
